    ...
    for i in range(0, len(x_hist)):
        if x_hist[i] != 0 and start == 0:
            ...
            if abs(i - start) > expected:
                ...
                x_segment.append(start - space if start - space > 0 else 0)
                x_segment.append(i + space if i + space < len(x_hist) else len(x_hist))
                start = 0
                x_sum = 0
            elif x_sum < filter:
                start = 0
    ...
    for i in range(0, len(y_hist)):
        if y_hist[i] != 0 and start == 0:
            ...
            if abs(i - start) > expected:
                ...
                y_segment.append(start - space if start - space > 0 else 0)
                y_segment.append(i + space if i + space < len(x_hist) else len(x_hist))
                start = 0
                y_sum = 0
            elif y_sum < filter:
                start = 0
...
def create_rect(x_hist, y_hist, image):
    ...
    for i in range(0, (len(x_segment)//2)):
        ...
        for j in range(0, (len(y_segment)//2)):
            ...
            x_temp, y_temp, nothing = create_histogram_from_array(crop)
...
def segementChara(filepath):
    image = preprocess(filepath)
    x_hist, y_hist, image = create_histogram_from_array(image)
    x_hist, y_hist, image = trim_img(x_hist, y_hist, image, 5, 50)
    return create_rect(x_hist, y_hist, image)

segementChara("../test_model/hello1.jpg")
